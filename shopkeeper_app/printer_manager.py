...
class PrinterManager:
    ...
    def _get_paper_status_from_flags(self, status_flags):
        """Decode Windows printer status flags into a paper status string and severity.

        Returns:
            (status_code, human_readable)
                status_code: 'ok', 'issue', or 'unknown'
        """
        try:
            # Standard Win32 PRINTER_STATUS_* flags (see winspool.h)
            ...
            issues = []
            if status_flags & PRINTER_STATUS_PAPER_JAM:
                issues.append("Paper jam")
            if status_flags & PRINTER_STATUS_PAPER_OUT:
                issues.append("Out of paper")
            if status_flags & PRINTER_STATUS_PAPER_PROBLEM:
                issues.append("Paper problem")

            # Treat some related conditions as "paper not ok" because printing cannot proceed
            if status_flags & PRINTER_STATUS_DOOR_OPEN:
                issues.append("Door open")
            if status_flags & PRINTER_STATUS_NOT_AVAILABLE:
                issues.append("Printer not available")

            if issues:
                return "issue", ", ".join(issues)

            # No explicit paper-related flags set
            return "ok", "Paper appears OK"

        except Exception as e:
            logger.debug(f"Error decoding paper status flags: {e}")
            return "unknown", "Unable to determine paper status"

    def get_printer_paper_status(self, printer_name):
        """Check low-level OS status flags for paper-related problems.
        ...
        Fail-safe:
            - If the printer cannot be opened or queried, returns 'unknown'.
        """
        try:
            if not printer_name:
                return "unknown", "No printer name provided"
            ...
            status_flags = info.get("Status") or 0
            return self._get_paper_status_from_flags(status_flags)

        except Exception as e:
            logger.warning(f"Failed to read paper status for printer {printer_name}: {e}")
            return "unknown", f"Unable to read printer status: {e}"
