document.addEventListener('DOMContentLoaded', function () {
    ...
    try {
        ...
        console.warn('Socket.IO not available, using fallback polling only:', error);
    }
    ...
    async function fetchPricing() {
        try {
            ...
            const response = await fetch(`/api/pricing/${shopId}`);
            if (response.ok) {
        ...
        const previewPromise = (async () => {
            ...
            try {
                ...
                const opts = {
                    ...
                    shop_id: document.querySelector('input[name="shop_id"]').value
                };
                ...
                formData.append('shop_id', opts.shop_id);
                formData.append('page_size', opts.page_size);
                formData.append('orientation', opts.orientation);
                formData.append('color_mode', opts.color_mode);
                formData.append('print_side', opts.print_side);
                formData.append('page_range', opts.page_range);
                formData.append('layout_pages', opts.layout_pages);
                formData.append('copies', opts.copies);
                formData.append('is_cropped', metadata.some(m => m.is_cropped));
                formData.append('crop_info', JSON.stringify(metadata));

                // Request preview with abort signal
                const response = await fetch('/api/preview', {
                    method: 'POST',
                    body: formData,
                    signal: xeroxPreviewAbortController.signal
                });
    ...
    async function submitXeroxPrintJob() {
        try {
            ...
            console.info(`XEROX: submitting ${files.length} captured images for shop ${shopId}`);

            // Create FormData
            const formData = new FormData();
            formData.append('shop_id', shopId);
            formData.append('source', 'xerox');

            // Append each captured image
            files.forEach((file, index) => {
                formData.append('file[]', file, `scan_${index + 1}.png`);
            });
            formData.append('is_cropped', metadata.some(m => m.is_cropped));
            formData.append('crop_info', JSON.stringify(metadata));

            // Add customization settings
            formData.append('copies', document.getElementById('xeroxCopies').value);
            formData.append('page_range', prVal);
            formData.append('page_size', document.getElementById('xeroxPageSize').value);
            formData.append('orientation', document.getElementById('xeroxOrientation').value);
            formData.append('print_side', document.getElementById('xeroxPrintSide').value);
            formData.append('color_mode', document.getElementById('xeroxColorMode').value);
            formData.append('layout_pages', document.getElementById('xeroxLayoutPages').value);
            formData.append('layout_type', (function (v) {
                switch (v) {
                    case '1': return 'normal';
                    case '2': return '2up';
                    case '4': return '4up';
            ...
            })(document.getElementById('xeroxLayoutPages').value));
            ...
            if (data.success) {
                ...
                // Hide preview and settings sections
                previewSection.style.display = 'none';
                xeroxSettingsSection.style.display = 'none';
    ...
    function showStatusSection() {
        ...
        const shopIdInput = document.querySelector('input[name="shop_id"]');
        currentShopId = shopIdInput ? shopIdInput.value : null;
    ...
    function startSingleJobPolling() {
        ...
        if (currentShopId && socket) {
            socket.emit('join_shop', { shop_id: currentShopId });
        }
    ...
    function fetchSingleJobStatus() {
        ...
        fetch(`/api/job/${currentJobId}/status`)
            .then(r => r.json())
            .then(data => {
                ...
                if (status.toLowerCase().includes('failed')) {
                    singleJobError.textContent = data.error_message || 'Job Failed';
                    singleJobError.style.display = 'block';
                    if (singleJobProgress) singleJobProgress.style.width = '100%';
                }
                // Normalize status for completion check
                let normalizedStatus = status;
                if (status === 'Printing Completed' || status.includes('Completed')) {
                    normalizedStatus = 'Completed';
                }

                if (normalizedStatus === 'Completed' || normalizedStatus.toLowerCase().includes('failed')) {
                    clearInterval(statusCheckInterval);
    ...
    if (socket) {
        socket.on('job_status_update', function (data) {
            if (data.job_id === currentJobId) {
                ...
                if (details && singleJobMeta) {
                    singleJobMeta.textContent = `${details}`;
                }

                if (status.toLowerCase().includes('failed')) {
                    singleJobError.textContent = details || 'Job Failed';
                    singleJobError.style.display = 'block';
                }
        ...
        socket.on('disconnect', function () {
            console.warn('Socket.IO disconnected, using fallback polling');
        });
    }
